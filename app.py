...
app = Flask(__name__)
secret_key = os.getenv('SECRET_KEY')  # Load secret key from .env file
if not secret_key:
...
class Document(db.Model):
    ...
    incoming_date = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)  # New column
    outgoing_date = db.Column(db.DateTime, nullable=False, default=datetime.utcnow)  # New column
...
def send_document_added_email(user, user_gmail, section, office, document_subject, document_category, tracking_no, receiving_office, receiving_section):
    ...
    view_file_url = f"http://dts-denr-v2.onrender.com/viewFile.html/{tracking_no}" #localhost dts-denr
    body = f"""
    <html>
    <body>
    ...
    if user:
        try:
            if bcrypt.check_password_hash(user.password, password):
                ...
                return redirect(url_for('all_documents', username=encrypted_username))  # Redirect to all documents
        except ValueError:
            return 'Invalid password format', 400
...
def add_document():
    try:
        ...
        app.logger.error(f"Error in add_document: {str(e)}")
        return jsonify({'success': False, 'error': str(e)}), 500
